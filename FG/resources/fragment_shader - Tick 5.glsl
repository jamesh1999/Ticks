...
float cube(vec3 pt) {
  vec3 ptAbs = abs(pt);
  return max(max(ptAbs.x, ptAbs.y), ptAbs.z) - 1;
}

float torus(vec3 pt) {
  vec2 q = vec2(length(pt.xy) - 3, pt.z);
  return length(q) - 1;
}
...
float shadow(vec3 pt, vec3 lpos) {
  ...
  for (float t=0.1; t < length(lpos - pt) && step < RENDER_DEPTH && kd > 0.001;) {
    float d = abs(geomOperations(pt + t * l));
    if (d < 0.001) {
      kd = 0;
    } else {
      kd = min(kd, 16 * d / t);
...
float shade(vec3 eye, vec3 pt, vec3 n) {
  ...
  for (int i = 0; i < LIGHT_POS.length(); i++) {
    ...
    val += (max(dt, 0) + max(pow(dot(r, v), 256), 0)) * shadow(pt, LIGHT_POS[i]);
  }
  return val;
}
