...
public class Renderer {
	...
	private ColorRGB illuminate(Scene scene, SceneObject object, Vector3 P, Vector3 N, Vector3 O)
	{
		...
		for (int i = 0; i < pointLights.size(); i++)
		{
			...
			{
				// Position on light to raycast too
				Vector3 lightPosition = light.getPosition().add(
					Vector3.randomInsideUnitSphere().scale(LIGHT_SIZE));
				...
				if(hit.getDistance() >= distanceToLight) visibility += 1.0 / SHADOW_RAY_COUNT;
			}
			...
			if (cosLN > 0.0) colourToReturn = colourToReturn.add(C_diff.scale(k_d).scale(I).scale(cosLN).scale(visibility));

			//Add specular light
			Vector3 R = N.scale(2.0 * cosLN).subtract(L);
			double cosRV = V.dot(R);
			if (cosRV > 0.0) colourToReturn = colourToReturn.add(C_spec.scale(k_s).scale(I).scale(Math.pow(cosRV, alpha)).scale(visibility));
		}
